...
### 5.4 cross-modal RoPE 对齐到底是什么

先把概念说清楚：

- 普通 RoPE 的作用，是把“位置”编码成 `q/k` 上的旋转相位
- 若 audio / video 各自只用自己的离散索引，那么“同一个真实时刻”的视频 token 与音频 token，可能会落在完全不相关的 RoPE 相位上
- 所谓 **cross-modal RoPE 对齐**，本质上就是：
  - 先把两种模态的 token 映射到**同一条时间轴**
  - 再分别基于这条共享时间轴生成 rotary `cos/sin`
  - 让 cross-attention 里的 `q(video)` 与 `k(audio)`，或者 `q(audio)` 与 `k(video)`，在“时间接近”时天然拥有更一致的相位关系

因此它不是“又加一个新 attention 层”，而是**在 cross-attention 前，把两种模态的位置编码体系先对齐**。

#### 5.4.1 MOVA 里是怎么实现的

`DualTowerConditionalBridge.build_aligned_freqs()` 的逻辑非常直接：

- 音频侧把 `audio_pos = [0, 1, 2, ..., L_a-1]` 当作参考时间轴
- 视频侧考虑到 video VAE temporal stride 是 `4`，所以 latent 帧率不是原始 `video_fps`，而是 `video_fps / 4`
- 然后按下面的比例把视频 latent 帧映射到音频 latent 步长单位：
  - `scale = audio_fps / (video_fps / 4.0)`
  - `video_pos_per_frame = arange(f_v) * scale`
- 再把每个 frame 的时间位置重复给该帧内全部 `h*w` 个视频 token：
  - 同一 latent frame 内的所有空间 token 共享同一个 cross-modal 时间相位
- 最后分别调用同一个 `RotaryEmbedding(base=10000, dim=head_dim)`，生成：
  - video 侧 `(cos_v, sin_v)`
  - audio 侧 `(cos_a, sin_a)`

这组对齐后的 `cos/sin` 会在 `ConditionalCrossAttention.forward()` 中分别作用到：

- 主塔的 query
- 条件塔的 key

也就是说，MOVA 的 bridge 做的其实是：

- **video / audio 各自保留本模态 self-attention 的位置编码**
- **只在 cross-attention 这条链路上，额外引入一套共享时间轴上的 RoPE**

还有两个实现细节值得注意：

- 当前 checkpoint 配置里 `apply_cross_rope=true`，所以这条路径确实启用
- 但 `apply_first_frame_bias_in_rope=false`，所以默认实现采用“均匀 stride=4 latent 时间轴”，没有把首帧的特殊时间偏置单独算进去

#### 5.4.2 它和 LTX-2.x / 当前仓库里 LTX-2.3 路线代码的关系

如果对照当前仓库 `models/LTX-2/packages/ltx-core` 里的实现，会发现两者的**设计目标其实非常接近**：

- 都是 dual-stream / dual-tower 风格的音视频联合扩散结构
- 都没有在 cross-modal 里做空间 RoPE 对齐，而是只保留 **temporal-only RoPE**
- 都试图让 audio/video cross-attention 建立在“同一真实时间轴”上，而不是让两个模态各自拿着互不相干的位置编号去做注意力

LTX 这条链路的实现位置更靠前一些：

- video 侧先通过 `VideoLatentPatchifier.get_patch_grid_bounds()` 得到 patch 的 `[start, end)` 时间边界
- 再通过 `get_pixel_coords(..., causal_fix=True)` 和 `positions[:, 0, ...] / fps` 把视频时间坐标换成秒
- audio 侧则在 `AudioPatchifier._get_audio_latent_time_in_sec()` 里，结合 `hop_length / sample_rate` 与 causal offset，直接生成秒级时间坐标
- 最后 `MultiModalTransformerArgsPreprocessor.prepare()` 在做 cross-attention positional embedding 时，只切出 `positions[:, 0:1, :]` 这一维，也就是只保留时间轴，并用 `use_middle_indices_grid=True` 取每个 `[start, end)` 区间的中点来生成 1D cross-modal RoPE

所以从方法论上讲：

- **MOVA 与 LTX 都属于“temporal cross-modal RoPE 对齐”这一类设计**
- MOVA 不是凭空多出一个完全不同的东西，而是把这件事写成了更集中、更工程化的一段 bridge 逻辑

#### 5.4.3 MOVA 和 LTX-2.3 在实现上的关键差异

虽然设计目标接近，但两者落地方式并不一样：

1. **时间坐标的单位不同**
  - MOVA 用的是“audio latent step”单位：`0,1,2,...`
  - LTX 用的是**真实时间秒数**，然后再通过 `max_pos` 归一化进 RoPE

2. **对齐发生的位置不同**
  - MOVA 在 bridge 前向里临时根据 `video_fps + grid_size + audio_steps` 现算一遍 aligned freqs
  - LTX 在 latent tools / patchifier 阶段就把时间坐标准备好，transformer 里只是消费这些坐标

3. **时间语义的精细度不同**
  - MOVA 给同一视频 latent frame 内的所有 `h*w` token 赋同一个时间点，本质上是“frame-level point timestamp”
  - LTX 先保留每个 patch 的 `[start, end)` 边界，再在 RoPE 里取中点，因此对 causal fix、patch 边界和真实时间跨度的表达更系统

4. **伴随机制不同**
  - MOVA 公开 bridge 的主控制量是 residual cross-attention，本身没有类似 LTX 那种显式 `cross_scale_shift_timestep / cross_gate_timestep`
  - LTX 会把另一模态的 `sigma` 送进 `cross-modality AdaLN`，同时调制 scale / shift / gate，因此它不只是“时间对齐”，还把**跨模态噪声阶段同步**一起建模了

5. **工程耦合方式不同**
  - MOVA 的写法更依赖当前模型固定超参，例如 video VAE temporal stride=`4`、bridge config 里的 `audio_fps=50.0`
  - LTX 的写法更通用，时间坐标来自 patchifier 与 latent shape，本身更像“统一的位置系统”

因此更准确的说法是：

- MOVA 的 cross-modal RoPE 对齐，与 LTX-2.x / 2.3 的 temporal cross-attention 是**同一设计家族**
- 但 MOVA 版本更像“把视频 latent 帧显式换算到音频 latent 50Hz 时间轴上”
- LTX 版本则更像“先在系统层面为两种模态建立秒级时间坐标，再在 cross-attention 中只取时间轴做 RoPE，并叠加 cross-modality AdaLN”
...
- 其中 cross-modal RoPE 的思路与当前仓库 `LTX-2` 路线属于同一类 temporal alignment 设计，但 MOVA 把对齐逻辑集中写在 bridge 内部，用 audio latent 步长单位做换算；LTX 则更系统地在 patchifier / latent tools 里先构造成秒级时间坐标，并再叠加 cross-modality AdaLN
