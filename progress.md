...
按用户确认清理 `Wan2.2-T2V`：复核当前仓库对 `models/Wan2.2/weights/Wan2.2-T2V-A14B` 的引用，删除 118G T2V 权重目录，清理残留 T2V 专用脚本与夜间队列引用，并同步更新说明文档。

### 结果与效果

- 已确认当前无运行中进程、无软链接、无其他正式 benchmark 链路依赖 `Wan2.2-T2V-A14B`；保留的仅是历史最小测试记录与未执行的 T2V 脚本路径。
- 已删除 `models/Wan2.2/weights/Wan2.2-T2V-A14B`，释放约 `118G`。
- 已删除 `test/wan2.2-t2v-i2v/test_wan22_t2v.sh`、`test/wan2.2-t2v-i2v/run_phase4_t2v_filtered.sh`、`test/phase4_tail_queue_longcat_wan22.sh`，避免仓库继续保留指向已删除权重的可执行入口。
- 已更新 `test/wan2.2-t2v-i2v/test.md`、`model.md`、`plan.md`，将 Wan2.2-T2V 状态统一改为“已清理”，同时保留历史最小测试结果说明。
- `df -h /root/autodl-tmp` 显示可用空间由清理前约 `35G` 提升到约 `153G`，磁盘使用率由 `98%` 降到 `90%`。
