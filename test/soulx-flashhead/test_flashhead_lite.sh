...
$PYTHON generate_video.py \
  ...
  --save_file $PROJ_ROOT/test/soulx-flashhead/output/lite/flashhead_lite_minimal.mp4
...
# 输出视频信息
if [ -f "$PROJ_ROOT/test/soulx-flashhead/output/lite/flashhead_lite_minimal.mp4" ]; then
  echo "输出文件: $(ls -lh $PROJ_ROOT/test/soulx-flashhead/output/lite/flashhead_lite_minimal.mp4)"
  ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 \
    $PROJ_ROOT/test/soulx-flashhead/output/lite/flashhead_lite_minimal.mp4 2>/dev/null | xargs -I{} echo "视频时长: {}s"
fi
echo "=== Lite 测试完成 ==="
