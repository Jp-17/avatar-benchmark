...
OUT_DIR=/root/autodl-tmp/avatar-benchmark/output/multitalk_newphase4
  ...
  cat > "$OUT_DIR/config.json" <<JSON
{
  ...
  "skipped_conditions": {
    "C_half_long": "MultiTalk 当前稳定路径为最小短时链路，长音频 filtered 条件尚未验证。",
    "C_full_long": "MultiTalk 当前稳定路径为最小短时链路，长音频 filtered 条件尚未验证。"
  }
}
JSON
}
...
- 实际命令：$cmd
- config 参数：见 output/multitalk_newphase4/config.json 与 output/multitalk_newphase4/logs/${cond}.json
- 输出路径：$out
- 显存峰值：${peak} MB
- 推理生成时间：${runtime} 秒
- 日志：$log
...
JSON
  ...
  set +e
  env CUDA_VISIBLE_DEVICES=0 LD_LIBRARY_PATH=/root/autodl-tmp/envs/unified-env/lib:/root/miniconda3/lib PYTHONPATH="$PY_DEPS:$ENV_SITE" /root/miniconda3/bin/python -S generate_multitalk.py --ckpt_dir weights/Wan2.1-I2V-14B-480P --wav2vec_dir weights/chinese-wav2vec2-base --input_json "$input_json" --size multitalk-480 --sample_steps 8 --mode streaming --motion_frame 9 --num_persistent_param_in_dit 0 --save_file "$prefix" >> "$log" 2>&1
  cmd_status=$?
  set -e
  end_ts=$(date +%s)
...
write_config_json
init_results_md
append_skip C_half_long "MultiTalk 当前稳定路径为最小短时链路，长音频 filtered 条件尚未验证。"
append_skip C_full_long "MultiTalk 当前稳定路径为最小短时链路，长音频 filtered 条件尚未验证。"
