...
OUT_DIR=/root/autodl-tmp/avatar-benchmark/output/soulx_flashtalk_newphase4
  ...
  cat > "$OUT_DIR/config.json" <<JSON
{
  ...
  "skipped_conditions": {
    "C_half_long": "参考 test/soulx-flashtalk/test.md 的最小稳定路径，本轮先完成短时子集。",
    "C_full_long": "参考 test/soulx-flashtalk/test.md 的最小稳定路径，本轮先完成短时子集。"
  }
}
JSON
}
...
run_case() {
  ...
  set +e
  /root/miniconda3/bin/conda run --no-capture-output -p "$ENV" env CUDA_VISIBLE_DEVICES=0 XFORMERS_IGNORE_FLASH_VERSION_CHECK=1 python generate_video.py --ckpt_dir /root/autodl-tmp/avatar-benchmark/models/SoulX-FlashTalk/weights/SoulX-FlashTalk-14B --wav2vec_dir /root/autodl-tmp/avatar-benchmark/models/SoulX-FlashTalk/models/chinese-wav2vec2-base --input_prompt "$prompt" --cond_image "$img" --audio_path "$audio" --audio_encode_mode stream --save_file "$out" --cpu_offload >> "$log" 2>&1
  cmd_status=$?
  set -e
  end_ts=$(date +%s)
...
write_config_json
init_results_md
append_skip C_half_long "参考 test/soulx-flashtalk/test.md 的最小稳定路径，本轮先完成短时子集。"
append_skip C_full_long "参考 test/soulx-flashtalk/test.md 的最小稳定路径，本轮先完成短时子集。"
