...
### C_half_long
- 状态：⏭️ skipped
- 跳过原因：Self-Forcing 为 text-only 模型，不支持 audio-driven；本轮 benchmark 中的长音频条件本身不适用，且当前稳定路径仅验证到 21 帧短视频。
...
- 失败经验与解决方法：沿用 test/self-forcing/test.md 中已验证的 sf-longlive-env 稳定路径，未新增异常。
