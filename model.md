...
## 五、Phase 2 环境与权重状态（2026-03-07 最终更新）

### Phase 2 完成状态总览

> 图例：✅ = 完成；⚠️ = 部分完成/有条件；❌ = 未完成

| # | 模型 | 环境名 | 环境创建 | 依赖安装 | 权重下载 | 可推理 | 已测试 | 备注 |
|---|------|--------|---------|---------|---------|--------|--------|------|
| 1 | EchoMimic v2 | echomimic2-env | ✅ | ✅ | ✅ 11G | ✅ | ✅ 12/12 | Phase 4 批推理全部完成 |
| 2 | StableAvatar | stableavatar-env | ✅ | ✅ | ✅ 26G | ✅ | ✅ 8/12 | C_en_3m OOM, 4条件未完成 |
| 3 | LiveTalk | livetalk-env | ✅ | ✅ | ✅ 23G | ✅ | ✅ 11/12 | C_en_5m 受 frame_seq_length=1024 限制 |
| 4 | Hallo3 | hallo3-env | ✅ | ✅ | ✅ 49G | ✅ | ✅ 3/10 | C_zh_5s/10s/30s done, GPU 冲突中断 |
| 5 | Ovi | ovi-env | ✅ | ✅ | ✅ 83G | ✅ | ✅ 1 test | qint8+cpu_offload, ~25.8G GPU |
| 6 | MOVA | mova-env (venv) | ✅ | ✅ | ✅ 73G | ✅ | ❌ | 测试脚本已创建，被 watchdog 干扰 |
| 7 | Wan2.2-S2V | wan2.2-env | ✅ | ✅ | ✅ 46G | ✅ | ❌ | 测试脚本已创建 |
| 8 | LiveAvatar | liveavatar-env | ✅ | ✅ | ✅ 47G | ✅ | ❌ | 测试脚本已创建 |
| 9 | SoulX-FlashTalk | flashtalk-env | ✅ | ✅ | ✅ 42.5G | ✅ | ❌ | 测试脚本已创建 |
| 10 | LTX-2 | .venv (uv) | ✅ | ✅ | ✅ 205G | ✅ | ❌ | 测试脚本已创建 |
| 11 | OmniAvatar | omniavatar-env | ✅ | ✅ | ✅ 66G | ✅ | ❌ | 测试脚本已创建, 24G 模式配置 |
| 12 | FantasyTalking | fantasy-talking-env | ✅ | ✅ | ✅ 32G | ✅ | ❌ | 测试脚本已创建, 24G 模式 |
| 13 | SkyReels-V3 | skyreels-env | ✅ | ⚠️ | ❌ ~2G/~30G | ❌ | ❌ | 权重未下载 |
| 14 | HunyuanVideo-Avatar | hunyuan-avatar-env | ✅ | ⚠️ | ❌ 14G/~50G | ❌ | ❌ | 权重不完整，下载进程中断 |
| 15 | HunyuanVideo-1.5 | ltx2-hunyuan15-env | ✅ | ⚠️ | ❌ 13M | ❌ | ❌ | 权重未下载 |
| 16 | LongLive | sf-longlive-env | ✅ | ⚠️ | ❌ 277M | ❌ | ❌ | 权重未下载 |
| 17 | LongCat-Video-Avatar | longcat-env | ✅ | ⚠️ | ❌ 495M | ❌ | ❌ | XetHub CDN 超时 |
| 18 | MultiTalk | unified-env | ✅ | ⚠️ | ❌ 6.3G/~50G | ❌ | ❌ | XetHub CDN 超时 |
| 19 | InfiniteTalk | unified-env | ✅ | ⚠️ | ❌ 21G/~115G | ❌ | ❌ | XetHub CDN 超时 |
| 20 | Self-Forcing | 无专用环境 | ❌ | ❌ | ❌ 143M | ❌ | ❌ | 需创建环境并下载权重 |
| 21 | Wan2.2 T2V/I2V | wan2.2-env | ✅ | ✅ | ⚠️ T2V已下 | ⚠️ | ❌ | T2V-14B ✅, I2V 权重待下载 |

### 统计

- **环境创建完成**：20/21（仅 Self-Forcing 无专用环境）
- **依赖安装完成**：12/21（含 flash_attn 分发）
- **权重下载完成**：12/21
- **可进行推理测试**：12/21
- **已进行推理测试**：5/21（EchoMimic v2, StableAvatar, LiveTalk, Hallo3, Ovi）
...
| 模型 | 缺失内容 | 预估需下载量 | 阻塞原因 |
|------|---------|------------|---------|
| SkyReels-V3 | 主体权重 | ~28G | 未启动下载 |
| HunyuanVideo-Avatar | 大部分权重 | ~36G | 下载进程中断 |
| HunyuanVideo-1.5 | 全部权重 | ~30G | 未启动下载 |
| LongLive | 全部权重 | ~5G | 未启动下载 |
| LongCat-Video-Avatar | 大部分权重 | ~25G | XetHub CDN 超时 |
| MultiTalk | 大部分权重 | ~44G | XetHub CDN 超时 |
| InfiniteTalk | 大部分权重 | ~94G | XetHub CDN 超时 |
| Self-Forcing | adapter + base model | ~15G | 需创建环境 |
| Wan2.2 I2V | I2V-14B 权重 | ~28G | 未启动下载 |
