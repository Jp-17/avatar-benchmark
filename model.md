...
|---|------|--------|---------|---------|---------|--------|--------|----------------|------|
| 1 | EchoMimic v2 | echomimic2-env | ✅ | ✅ | ✅ 11G | ✅ | ✅ 1 test | ✅ 新4条件完成 | 已完成新 Phase 4 的 C_half_short/C_half_long/C_full_short/C_full_long |
| 2 | StableAvatar | stableavatar-env | ✅ | ✅ | ✅ 26G | ✅ | ✅ 1 test | ✅ 新4条件完成 | 已完成新 Phase 4 的 C_half_short/C_half_long/C_full_short/C_full_long |
...
| 4 | Hallo3 | hallo3-env | ✅ | ✅ | ✅ 49G | ✅ | ✅ 1 test | ✅ 支持子集完成 | 已完成新 Phase 4 的 C_half_short/C_full_short；C_half_long/C_full_long 按最小测试稳定路径跳过；results.md 已记录显存峰值与生成时间 |
| 5 | Ovi | ovi-env | ✅ | ✅ | ✅ 83G | ✅ | ✅ 1 test | ✅ 支持子集完成 | 已完成新 Phase 4 的 C_half_short/C_full_short；C_half_long/C_full_long 按最小测试稳定路径跳过；results.md 已记录显存峰值与生成时间 |
| 6 | MOVA | mova-env (venv) | ✅ | ✅ | ✅ 73G | ✅ | ✅ 1 test | 无 | 最小推理已通过；修正版输出已调整为 512x512 |
| 7 | Wan2.2-S2V | wan2.2-env | ✅ | ✅ | ✅ 46G | ✅ | ✅ 1 test | 无 | 最小推理已通过 |
| 8 | LiveAvatar | liveavatar-env | ✅ | ✅ | ✅ 47G | ✅ | ✅ 1 test | 无 | 已生成最小输出；80帧 GPU 实验可进入推理，但与 Phase 4 并行时因显存不足 OOM |
| 9 | SoulX-FlashTalk | flashtalk-env | ✅ | ✅ | ✅ 42.5G | ✅ | ✅ 1 test | 无 | 最小推理已通过；修正版输出已调整为 640x640，且已补回音轨 |
| 10 | LTX-2 | .venv (uv) | ✅ | ✅ | ✅ 205G | ✅ | ✅ 1 test | ✅ 支持子集完成 | 已完成新 Phase 4 的 C_half_short/C_full_short；C_half_long/C_full_long 按最小测试稳定路径跳过；results.md 已记录显存峰值与生成时间 |
| 11 | OmniAvatar | omniavatar-env | ✅ | ✅ | ✅ 66G | ✅ | ✅ 1 test | 无 | 最小推理已通过；补装 imageio-ffmpeg 并改为使用 PATH 中的 ffmpeg 后完成输出 |
| 12 | FantasyTalking | fantasy-talking-env | ✅ | ✅ | ✅ 32G | ✅ | ✅ 1 test | ✅ 支持子集完成 | 已完成新 Phase 4 的 C_half_short/C_full_short；C_half_long/C_full_long 按最小测试稳定路径跳过；results.md 已记录显存峰值与生成时间 |
...
| 16 | LongLive | sf-longlive-env | ✅ | ✅ | ✅ 8.2G + 共享 Wan2.1-T2V-1.3B | ✅ | ✅ 1 test | 无 | 最小推理已通过；复用 shared Wan 基座 |
| 17 | LongCat-Video-Avatar | longcat-env | ✅ | ⚠️ | ⚠️ base约73G/avatar约77G | ❌ | ❌ | 优先-1 | LongCat-Video base 已基本下完；Avatar 下载因 shard 校验失败中断，需清理损坏分片后续传 |
| 18 | MultiTalk | unified-env | ✅ | ✅ | ✅ 最小推理所需权重已齐 | ✅ | ✅ 1 test | 无 | 最小推理已通过；模型局部 overlay 接入 MultiTalk index + `multitalk.safetensors`，输出 398K MP4，runtime 2053s |
| 19 | InfiniteTalk | unified-env | ✅ | ✅ | ✅ 最小推理所需权重已齐 | ✅ | ✅ 1 test | 无 | 最小推理已通过；补上 image-input 场景的 ffprobe fallback 后输出 356K MP4，runtime 1030s |
| 20 | Self-Forcing | sf-longlive-env（复用） | ✅ | ✅ | ✅ 5.3G + 共享 Wan2.1-T2V-1.3B | ✅ | ✅ 1 test | 无 | 最小推理已通过；av 降级到 12.3.0 解决写视频报错 |
...
| LongCat-Video-Avatar | 大部分权重 | ~25G | 当前最高优先推进 |
| Wan2.2 I2V | I2V-14B 权重 | ~22G | 用户要求暂缓（保留已下载 49G） |

### 当前优先级（2026-03-08 更新）

- **优先推进**：LongCat-Video-Avatar 的权重补齐与最小素材推理测试
- **已完成**：MultiTalk / InfiniteTalk 已补齐 shared Wan2.1-I2V-14B-480P，并通过 Phase 2 最小素材推理验证
