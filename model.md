...
## 五、Phase 2/P3/P4 环境与权重状态（2026-03-06 18:10 更新）

### 环境依赖安装：全部完成（16/16）

| # | 环境名 | Torch | 关键依赖 | 使用模型 | 权重状态 |
|---|--------|-------|---------|---------|---------|
| 1 | echomimic2-env | 2.5.1+cu121 | ✅ 全部 | EchoMimic v2 | ✅ 11G |
| 2 | stableavatar-env | 2.7.0+cu128 | ✅ 全部 | StableAvatar | ✅ 26G |
| 3 | livetalk-env | 2.10.0+cu128 | ✅ 全部 | LiveTalk | ✅ 23G |
| 4 | unified-env | 2.5.1+cu121 | ✅ 全部 | SoulX-FlashTalk, MultiTalk, InfiniteTalk | 🔄 下载中 |
| 5 | sf-longlive-env | 2.5.1+cu121 | ✅ torch+diffusers+transformers | Self-Forcing, LongLive | 🔄 下载中 |
| 6 | fantasy-talking-env | 2.5.1+cu121 | ✅ transformers==4.46.2 | FantasyTalking | ✅ 3.9G |
| 7 | liveavatar-env | 2.5.1+cu121 | ✅ transformers+diffusers (需flash_attn) | LiveAvatar | ✅ 8.9G (LoRA) |
| 8 | hallo3-env | 2.4.0+cu121 | ✅ diffusers+transformers+mediapipe+onnxruntime | Hallo3 | ✅ 54G |
| 9 | omniavatar-env | 2.5.1+cu121 | ✅ transformers+peft+einops (xfuser --no-deps) | OmniAvatar | ✅ 4.1G |
| 10 | hunyuan-avatar-env | 2.5.1+cu121 | ✅ diffusers==0.33.0+accelerate==1.1.1 | HunyuanVideo-Avatar | 🔄 14G/~40G |
| 11 | longcat-env | 2.6.0+cu124 | ✅ diffusers==0.35.1+transformers==4.41.0 (需flash_attn) | LongCat-Video-Avatar | ❌ 18G (XetHub) |
| 12 | wan2.2-env | 2.5.1+cu121 | ✅ diffusers+transformers+accelerate (需flash_attn) | Wan2.2-S2V/T2V | 🔄 下载中 |
| 13 | ovi-env | 2.5.1+cu121 | ✅ diffusers+transformers+accelerate+open-clip-torch | Ovi | 🔄 4.7G/~20G |
| 14 | mova-env (venv) | 2.5.1+cu121 | ✅ diffusers+transformers+accelerate+mmengine | MOVA | 🔄 3.4G/~20G |
| 15 | skyreels-env (venv) | 2.5.1+cu121 | ✅ diffusers==0.34.0+transformers==4.53.2 (需flash_attn) | SkyReels-V3 | ❌ 未下载 |
| 16 | ltx2-hunyuan15-env (venv) | 2.5.1+cu121 | ✅ diffusers==0.35.0+peft==0.17.0 | LTX-2, HunyuanVideo-1.5 | ❌ 未下载 |

### 可进行 inference 测试的模型（环境+权重全部就绪）

| # | 模型 | 环境 | 备注 |
|---|------|------|------|
| 1 | EchoMimic v2 | echomimic2-env | 已验证可运行 |
| 2 | StableAvatar | stableavatar-env | 已验证可运行 |
| 3 | LiveTalk | livetalk-env | 已验证可运行 |
| 4 | FantasyTalking | fantasy-talking-env | 可测（SSH 直接访问 GPU） |
| 5 | LiveAvatar | liveavatar-env | 需安装 flash_attn（可通过 SSH nohup 安装） |
| 6 | OmniAvatar | omniavatar-env | 可测 |
| 7 | Hallo3 | hallo3-env | 可测（需 CPU offload+VAE tiling） |

### 权重下载进行中

| 模型 | 当前大小 | 预估总大小 | 状态 |
|------|---------|-----------|------|
| Self-Forcing (adapter + Wan2.1-T2V-1.3B) | ~3.3G | ~5G | 🔄 下载中 |
| LongLive-1.3B | ~1.4G | ~5G | 🔄 下载中 |
| Ovi | ~4.7G | ~20G | 🔄 下载中 |
| HunyuanVideo-Avatar | ~14G | ~30-40G | 🔄 下载中 |
| Wan2.2-T2V-A14B | ~5.7G | ~30G | 🔄 下载中 |
| Wan2.2-S2V-14B | 刚启动 | ~30G | 🔄 下载中 |
| MOVA-360p | ~3.4G | ~20G | 🔄 下载中 |
| SoulX-FlashTalk (resume) | 44G | ~50G | 🔄 恢复中 |
| Wan2.1-I2V-14B-480P (共享) | ~9.8G | ~27G | 🔄 下载中 |

### 权重下载待启动

| 模型 | 预估大小 | 备注 |
|------|---------|------|
| MultiTalk (resume) | 46G → ~50G | XetHub CDN 超时问题 |
| InfiniteTalk (resume) | 103G → ~120G | XetHub CDN 超时问题 |
| LongCat-Video-Avatar (resume) | 18G → ~30G | XetHub CDN 超时问题 |
| LTX-2 | ~20G | 未启动 |
| HunyuanVideo-1.5 | ~30G | 未启动 |
| SkyReels-V3 | ~20G | 未启动 |

### flash_attn 待安装列表（需 JupyterLab 终端）

| 环境 | 安装命令 |
|------|---------|
| liveavatar-env | `conda activate /root/autodl-tmp/envs/liveavatar-env && pip install flash-attn --no-build-isolation` |
| wan2.2-env | `conda activate /root/autodl-tmp/envs/wan2.2-env && pip install flash-attn --no-build-isolation` |
| longcat-env | `conda activate /root/autodl-tmp/envs/longcat-env && pip install flash-attn --no-build-isolation` |
| unified-env | `conda activate /root/autodl-tmp/envs/unified-env && pip install flash-attn --no-build-isolation` |
| skyreels-env | `source /root/autodl-tmp/envs/skyreels-env/bin/activate && pip install flash-attn==2.7.4.post1 --no-build-isolation` |

### 注意事项

- 数据盘：481G/1000G (49%)，空间充足
- HF 下载: 勿用 network_turbo，仅设 `HF_ENDPOINT=https://hf-mirror.com`
- GPU 可通过 SSH 直接访问（已验证，nvidia-smi 正常，A800-SXM4-80GB 80GB VRAM）
- pip 安装 torch 依赖时需用 constraints 文件防止 torch 被覆盖
- XetHub CDN repos (MultiTalk/InfiniteTalk/LongCat-Video-Avatar) 有超时问题


### Phase 4 推理执行状态（2026-03-06）

#### GPU 访问确认
- **GPU**: NVIDIA A800-SXM4-80GB，80GB VRAM（SSH 直接可用，无需 JupyterLab）
- **确认时间**: 2026-03-06

#### LiveTalk 推理进度

| Condition | 状态 | 输出文件 | 时长 | 备注 |
|-----------|------|---------|------|------|
...
| C_en_5m   | retry | output/livetalk/C_en_5m.mp4 | ~254s | **最大支持254s**（frame_seq_length=1024限制） |
| C_sing_zh | done  | output/livetalk/C_sing_zh.mp4 | ~11s | 文件名：S001_jaychou.wav |
| C_sing_en | done  | output/livetalk/C_sing_en.mp4 | ~8s  | 文件名：S002_adele.wav |

**关键发现**：LiveTalk 最大支持 video_duration=254（≈4m14s），frame_seq_length=1024 不可修改（等于每帧空间 token 数，修改会破坏 KV-cache 位置计算）。

#### StableAvatar 推理进度

| Condition | 状态 | 输出文件 | 时长 |
|-----------|------|---------|------|
| C_zh_5s   | done | output/stableavatar/C_zh_5s.mp4  | ~5s  |
| C_zh_10s  | done | output/stableavatar/C_zh_10s.mp4 | ~10s |
| C_zh_30s  | done | output/stableavatar/C_zh_30s.mp4 | ~30s |
| C_zh_1m   | done | output/stableavatar/C_zh_1m.mp4  | ~60s |
| C_en_5s   | done | output/stableavatar/C_en_5s.mp4  | ~5s  |
| C_en_10s  | done | output/stableavatar/C_en_10s.mp4 | ~10s |
| C_en_30s  | done | output/stableavatar/C_en_30s.mp4 | ~30s |
| C_en_1m   | running | — | — |
| C_en_3m ~ C_sing_en | pending | — | — |

**说明**：StableAvatar 推理输出 video_without_audio.mp4，需后处理 ffmpeg 合并音频。每个 5s 条件约耗时 10 min（50 steps）。
