...
| 12 | FantasyTalking | fantasy-talking-env | ✅ | ✅ | ✅ 32G | ✅ | ✅ 1 test | 无 | 最小推理已通过 |
...
| 18 | MultiTalk | unified-env | ✅ | ⚠️ | ⚠️ 自身权重已到位 / 共享Wan未完 | ❌ | ❌ | 优先-2 | `multitalk.safetensors` 已下载完成；当前只剩 shared Wan 7 个 diffusion shards |
| 19 | InfiniteTalk | unified-env | ✅ | ⚠️ | ❌ 21G/~115G | ❌ | ❌ | 优先-3 | 已有 single/multi checkpoint，shared wav2vec 已验证可加载；当前还缺 7 个 Wan diffusion shards |
...
- **环境创建完成**：21/21
- **依赖安装完成**：14/21（含 flash_attn 分发）
- **权重下载完成**：14/21
- **可进行推理测试**：14/21
- **已进行推理测试**：8/21（EchoMimic v2, StableAvatar, LiveTalk, Hallo3, Ovi, Self-Forcing, LongLive, Wan2.2）
...
| MultiTalk | 大部分权重 | ~44G | 当前第二优先推进 |
| InfiniteTalk | 大部分权重 | ~94G | 当前第三优先推进 |
