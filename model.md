...
| 22  | SoulX-FlashHead(Pro/Lite) | Avatar 自回归 | 1.3B       | Lite 8–12GB / Pro 15–20GB | Yes  | Yes(HF) | 无限(streaming)   | ✅ 新4条件完成     | 已完成 Lite+Pro 各 4 条件；输出目录与 results.md 已校验；说话头专用（portrait），全身图需 `--use_face_crop True`                          | **可测**   |
...
| 22  | SoulX-FlashHead(Pro/Lite) | flashhead-env   | ✅    | ✅    | ✅ ~14G                       | ✅   | ✅ 2 test | ✅ 新4条件完成   | Lite 174s / Pro 283s 最小测试通过；Phase 4 Lite+Pro 各 4 条件全量完成；`XFORMERS_IGNORE_FLASH_VERSION_CHECK=1` 必须设置（xformers 0.0.31 与 flash_attn 2.8.3 版本绕过）；`--use_face_crop True` 全身图处理 |
