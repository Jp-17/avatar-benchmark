...
| 22  | SoulX-FlashHead(Pro/Lite) | Avatar 自回归 | 1.3B       | Lite 8–12GB / Pro 15–20GB | Yes  | Yes(HF) | 无限(streaming)   | 待配置            | 说话头模型（portrait 专用）；Pro/Lite 共享 1.3B DiT；`--use_face_crop` 支持全身图输入                                              | **可测**   |
...
| 22  | SoulX-FlashHead(Pro/Lite) | flashhead-env   | ❌    | ❌    | ❌ 待下载（~14.3G）              | ❌   | ❌        | ❌ 待配置      | 1.3B 说话头模型；flashtalk-env transformers 版本不兼容，需独立新环境；Pro ~15–20GB / Lite ~8–12GB |
