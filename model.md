...
| 9   | SoulX-FlashTalk      | flashtalk-env       | ✅    | ✅    | ✅ 42.5G                     | ✅   | ✅ 1 test | ✅ 支持子集完成   | 已完成新 Phase 4 的 C_half_short/C_full_short；保存逻辑已修正并沿用 cpu_offload 短时稳定路径；长时条件按当前稳定路径跳过                                                            |
| 10  | LTX-2                | .venv (uv)          | ✅    | ✅    | ✅ 205G                      | ✅   | ✅ 1 test | ✅ 支持子集完成   | 已完成新 Phase 4 的 C_half_short/C_full_short；长时条件按最小测试稳定路径跳过；results.md 已记录显存峰值与生成时间                                                                |
| 11  | OmniAvatar           | omniavatar-env      | ✅    | ✅    | ✅ 66G                       | ✅   | ✅ 1 test | ⚠️ 部分完成待复跑 | 已回收 `C_half_short` 输出；脚本已修正为递归识别嵌套 `demo_out` 目录，待补跑 `C_full_short`                                                                             |
...
| 19  | InfiniteTalk         | unified-env         | ✅    | ✅    | ✅ 最小推理所需权重已齐                | ✅   | ✅ 1 test | ✅ 支持子集完成 | 已完成新 Phase 4 的 C_half_short/C_full_short；沿用 image-input fallback + streaming 稳定路径，长时条件按当前稳定路径跳过                                  |
