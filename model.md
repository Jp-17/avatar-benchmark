...
| 7 | LTX-2 | 音视频联合 | 19B | 28GB(distil)/40GB(full) | Yes | Yes(HF) | 待确认(长) | ▶️ 进行中 | 2026-03-08 16:49 起执行短时子集；当前已生成阶段性 mp4，待 results.md / 文档回填后再标记完成 | **可测** |
...
| 14 | MultiTalk | Avatar 离线 | 14B | 24GB(低显存模式) | Yes | Yes(HF) | 15s+(streaming) | ⏳ 优先队列待执行 | 已在优先队列中，位于 LTX-2 之后等待执行 | **可测** |
...
| 21 | InfiniteTalk | Avatar 离线 | 14B | 24GB(低显存)/80GB | Yes | Yes(HF) | 无限(default 40s) | ⏳ 优先队列待执行 | 已在优先队列中，位于 MultiTalk 之后等待执行 | **可测** |
...
| 10 | LTX-2 | .venv (uv) | ✅ | ✅ | ✅ 205G | ✅ | ✅ 1 test | ▶️ 进行中 | 2026-03-08 16:49 起执行短时子集；当前已生成阶段性 mp4，待 results.md / 文档回填后再标记完成 |
| 11 | OmniAvatar | omniavatar-env | ✅ | ✅ | ✅ 66G | ✅ | ✅ 1 test | ⏳ 已入队 | 已加入 follow-up 队列，等待优先队列完成后执行 |
...
| 16 | LongLive | sf-longlive-env | ✅ | ✅ | ✅ 8.2G + 共享 Wan2.1-T2V-1.3B | ✅ | ✅ 1 test | ⏳ 已入队 | 已加入 follow-up 队列，位于 OmniAvatar 之后执行 |
...
| 18 | MultiTalk | unified-env | ✅ | ✅ | ✅ 最小推理所需权重已齐 | ✅ | ✅ 1 test | ⏳ 优先队列待执行 | 已在优先队列中，位于 LTX-2 之后等待执行 |
| 19 | InfiniteTalk | unified-env | ✅ | ✅ | ✅ 最小推理所需权重已齐 | ✅ | ✅ 1 test | ⏳ 优先队列待执行 | 已在优先队列中，位于 MultiTalk 之后等待执行 |
| 20 | Self-Forcing | sf-longlive-env（复用） | ✅ | ✅ | ✅ 5.3G + 共享 Wan2.1-T2V-1.3B | ✅ | ✅ 1 test | ⏳ 已入队 | 已加入 follow-up 队列，位于 LongLive 之后执行 |
...
#### 当前 Phase 4 总览（2026-03-08 16:53）
...
| LTX-2 | ▶️ 进行中 | 当前优先队列正在执行，已生成阶段性 mp4。 |
| MultiTalk | ⏳ 优先队列待执行 | 位于 LTX-2 之后。 |
| InfiniteTalk | ⏳ 优先队列待执行 | 位于 MultiTalk 之后。 |
| OmniAvatar | ⏳ follow-up 待执行 | 将在优先队列结束后启动。 |
