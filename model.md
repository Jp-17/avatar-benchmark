...
## 五、Phase 2/P3/P4 环境与权重状态（2026-03-06 17:30 更新）

### 可进行 inference 测试的模型（环境+权重完成）

| # | 模型 | Conda 环境 | Torch | 依赖 | 权重 | 备注 |
|---|------|-----------|-------|------|------|------|
| 1 | EchoMimic v2 | echomimic2-env | 2.5.1+cu121 | ✅ | 11G ✅ | 已验证可运行 |
| 2 | StableAvatar | stableavatar-env | 2.7.0+cu128 | ✅ | 26G ✅ | 已验证可运行 |
| 3 | LiveTalk | livetalk-env | 2.10.0+cu128 | ✅ | 23G ✅ | 已验证可运行 |
| 4 | FantasyTalking | fantasy-talking-env | 2.5.1+cu121 | ✅ (transformers==4.46.2) | 3.7G ✅ | 可测试（需 JupyterLab GPU） |
| 5 | LiveAvatar | liveavatar-env | 2.5.1+cu121 | ✅ (transformers==4.51.3, diffusers==0.37.0) | 1.3G ✅ (LoRA) | 需 JupyterLab 安装 flash_attn |
| 6 | hallo3 | hallo3-env | 2.4.0+cu121 | ❌ 需安装 | 49G ✅ | torch 已装，需安装模型依赖 |

### Torch 已装 + 权重完成（需安装模型依赖）

| 模型 | 环境 | Torch | 权重 | 待办 |
|------|------|-------|------|------|
| OmniAvatar | omniavatar-env | 2.5.1+cu121 | 1.2G ✅ | deps 安装被中断（xfuser 拉了错误 torch），需重装 deps（不含 xfuser） |

### Torch 已装 + 权重下载中/需重启

| 模型 | 环境 | Torch | 权重进度 | 下载状态 |
|------|------|-------|---------|---------|
| SoulX-FlashTalk | unified-env | 2.5.1+cu121 | 35G/~30G | 🔄 下载中（可能接近完成） |
| MultiTalk | unified-env | 2.5.1+cu121 | 46G | ❌ 进程死亡，需重启 |
| InfiniteTalk | unified-env | 2.5.1+cu121 | 103G | ❌ 进程死亡，需重启 |
| HunyuanVideo-Avatar | hunyuan-avatar-env | 2.5.1+cu121 | 8G | ❌ 进程死亡，需重启 |
| LongCat-Video-Avatar | longcat-env | 2.6.0+cu124 | 17G | ❌ 进程死亡（XetHub 超时） |
| Wan2.1-I2V-14B-480P (共享) | - | - | 877M | ❌ 进程死亡，需重启（MultiTalk+InfiniteTalk 共享） |

### Torch 已装 + 权重未下载

| 模型 | 环境 | Torch | 需下载权重 | 预估大小 |
|------|------|-------|-----------|---------|
| Self-Forcing | sf-longlive-env | 2.5.1+cu121 | gdhe17/Self-Forcing + Wan2.1-T2V-1.3B | ~5G |
| LongLive | sf-longlive-env | 2.5.1+cu121 | Efficient-Large-Model/LongLive-1.3B + Wan2.1-T2V-1.3B | ~5G |
| Wan2.2-T2V | wan2.2-env | 2.5.1+cu121 | Wan-AI/Wan2.2-T2V-A14B | ~30G |
| Wan2.2-S2V | wan2.2-env | 2.5.1+cu121 | 共享 LiveAvatar 的 Wan2.2-S2V-14B | ~30G |
| OVI | ovi-env | 2.5.1+cu121 | chetwinlow1/Ovi | ~20G |
| MOVA | mova-env (venv) | 2.5.1+cu121 | OpenMOSS-Team/MOVA-360p | ~20G |

### Torch 未装

| 模型 | 环境 | 需装 Torch | 权重状态 |
|------|------|-----------|---------|
| LTX-2 | ltx2-hunyuan15-env (venv) | 2.7.1+cu126 | 862M（部分） |
| HunyuanVideo-1.5 | ltx2-hunyuan15-env (venv) | 2.7.1+cu126 | 未下载 |
| SkyReels-V3 | skyreels-env (venv) | 2.8.0+cu126 | 682M（部分） |

### 环境汇总表

| 环境名 | Python | Torch | 状态 | 使用模型 |
|--------|--------|-------|------|---------|
| echomimic2-env | 3.10 | 2.5.1+cu121 | ✅ 完成 | EchoMimic v2 |
| stableavatar-env | 3.10 | 2.7.0+cu128 | ✅ 完成 | StableAvatar |
| livetalk-env | 3.10 | 2.10.0+cu128 | ✅ 完成 | LiveTalk |
| unified-env | 3.10 | 2.5.1+cu121 | ✅ 完成 | SoulX-FlashTalk, MultiTalk, InfiniteTalk |
| sf-longlive-env | 3.10 | 2.5.1+cu121 | ✅ torch+diffusers+transformers | Self-Forcing, LongLive |
| liveavatar-env | 3.10 | 2.5.1+cu121 | ✅ 全部依赖已装 | LiveAvatar |
| fantasy-talking-env | 3.10 | 2.5.1+cu121 | ✅ 全部依赖已装 | FantasyTalking |
| hallo3-env | 3.10 | 2.4.0+cu121 | ⚠️ torch 已装，模型 deps 未装 | Hallo3 |
| omniavatar-env | 3.10 | 2.5.1+cu121 | ⚠️ deps 安装被中断 | OmniAvatar |
| hunyuan-avatar-env | 3.10 | 2.5.1+cu121 | ⚠️ torch 已装，deps 未装 | HunyuanVideo-Avatar |
| longcat-env | 3.10 | 2.6.0+cu124 | ⚠️ torch 已装，deps 未装 | LongCat-Video-Avatar |
| wan2.2-env | 3.10 | 2.5.1+cu121 | ⚠️ torch 已装，flash_attn 需 JupyterLab | Wan2.2-S2V, Wan2.2-T2V |
| ovi-env | 3.10 | 2.5.1+cu121 | ⚠️ torch 已装，deps 未装 | OVI |
| mova-env | 3.10 | 2.5.1+cu121 | ⚠️ torch 已装，deps 未装 | MOVA |
| ltx2-hunyuan15-env | 3.10 | ❌ | ❌ torch 未装 | LTX-2, HunyuanVideo-1.5 |
| skyreels-env | 3.10 | ❌ | ❌ torch 未装 | SkyReels-V3 |

### flash_attn 待安装列表（需 JupyterLab 终端）

| 模型 | 环境 | 安装命令 |
|------|------|---------|
| LiveAvatar | liveavatar-env | `conda activate /root/autodl-tmp/envs/liveavatar-env && pip install flash-attn --no-build-isolation` |
| Wan2.2-S2V/T2V | wan2.2-env | `conda activate /root/autodl-tmp/envs/wan2.2-env && pip install flash-attn --no-build-isolation` |
| LongCat-Video-Avatar | longcat-env | `conda activate /root/autodl-tmp/envs/longcat-env && pip install flash-attn --no-build-isolation` |
| InfiniteTalk | unified-env | `conda activate /root/autodl-tmp/envs/unified-env && pip install flash-attn --no-build-isolation` |
| SkyReels-V3 | skyreels-env | `conda activate /root/autodl-tmp/envs/skyreels-env && pip install flash-attn==2.7.4.post1 --no-build-isolation` |
