...
| 18  | LiveAvatar           | Avatar 自回归 | 14B        | 48GB(FP8)/80GB(BF16)     | Yes  | Yes(HF) | 无限(10000s+)     | ✅ 支持子集完成     | 已完成新 Phase 4 的 C_half_short/C_full_short；当前真实最小测试基线为 80 帧 + offload_kv_cache + compile-disable 稳定路径；长时条件按该稳定路径暂跳过 | **条件可测** |
...
| 8   | LiveAvatar           | liveavatar-env      | ✅    | ✅    | ✅ 47G                       | ✅   | ✅ 1 test | ✅ 支持子集完成   | 已建立真实 80 帧最小基线（test/liveavatar/output/liveavatar_minimal_80gpu.mp4），并完成新 Phase 4 的 C_half_short/C_full_short；长时条件按当前稳定路径跳过                      |
| 9   | SoulX-FlashTalk      | flashtalk-env       | ✅    | ✅    | ✅ 42.5G                     | ✅   | ✅ 1 test | ✅ 支持子集完成   | 已完成新 Phase 4 的 C_half_short/C_full_short；另有长音频探针：`C_full_long` 实际约 60s 成功、`C_half_long` 失败；旧 results.md 中 `1.000s` 时长字段来自脚本统计错误 |
| 10  | LTX-2                | .venv (uv)          | ✅    | ✅    | ✅ 205G                      | ✅   | ✅ 1 test | ✅ 支持子集完成   | 已完成新 Phase 4 的 C_half_short/C_full_short；长时条件按最小测试稳定路径跳过；results.md 已记录显存峰值与生成时间                                                                |
| 11  | OmniAvatar           | omniavatar-env      | ✅    | ✅    | ✅ 66G                       | ✅   | ✅ 1 test | ✅ 支持子集完成   | 已完成 `C_half_short` / `C_full_short`；长音频条件尚未执行，当前稳定链路的单个 short case 已耗时约 3679s / 5414s，后续需单独排队 |
...
| 17  | LongCat-Video-Avatar | longcat-env         | ✅    | ✅    | ✅ base可用 + avatar 6/6       | ✅   | ✅ 1 test | ✅ 支持子集完成   | 已完成 `C_half_short` / `C_full_short`；长音频条件尚未执行，当前仅验证了短时 ai2v 稳定链路 |
...
| 19  | InfiniteTalk         | unified-env         | ✅    | ✅    | ✅ 最小推理所需权重已齐                | ✅   | ✅ 1 test | ✅ 支持子集完成   | 已完成新 Phase 4 的 C_half_short/C_full_short；长音频条件尚未执行，后续将沿用 image-input fallback + streaming 稳定路径单独排队 |
...
| LiveAvatar 原始音频时长补跑 | 暂停 | `C_half_short` 以 `infer_frames=136` 启动后长时间停在 `complete prepare conditional inputs`，无 mp4 落盘。 |
| Wan2.2-S2V 原始音频时长补跑 | 未开始 | 被 LiveAvatar full-audio 阻塞，当前尚未进入。 |
| LTX-2 原始音频时长补跑 | 未开始 | 被 LiveAvatar full-audio 阻塞，当前尚未进入。 |
| FantasyTalking 原始音频时长补跑 | 未开始 | 被 LiveAvatar full-audio 阻塞，当前尚未进入。 |
