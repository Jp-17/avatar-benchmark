...
**目标**：从各模型官方网站/demo页爬取示例素材，经预处理后建立标准化测试素材库，覆盖 info.md 中规定的素材类型。
...
| 类别 | 要求 | 数量目标 |
|------|------|----------|
| Audio - 讨论/对话 | 清晰人声，时长 10-30s，中英文各备 | ≥4 条 |
| Audio - 唱歌 | 人声歌唱，时长 10-30s | ≥2 条 |
...
`input.md` 记录所有可用素材及测试条件组合（condition），格���如下：
...
| ID  | 文件路径 | 类别 | 时长 | 语言 | 描述 |
|-----|----------|------|------|------|------|
| A001 | audio/speech/A001.wav | 讨论 | 15s | 中文 | 女声，正常语速，新闻播报风格 |
...
...
|-------------|----------|-------|-------|--------|------|------|
| C001 | image+audio→video | A001 | I001 | - | 15s | 半身像+中文演讲音频 |
| C002 | image+audio→video | A002 | I003 | - | 15s | 全身像+英文音频 |
| C003 | image+audio→video | A003 | I001 | - | 20s | 半身像+唱歌音频 |
| C004 | text+image→video | - | I001 | P001 | 10s | 半身像+演讲prompt |
| C005 | text+image→video | - | I003 | P004 | 10s | 全身像+跳舞prompt |
| C006 | text→video | - | - | P003 | 10s | 纯文本弹琴 |
...
```

**注意**：每个模型根据其支持的输入模态，选用对应的 Condition：
- 支持 image+audio→video 的模型 → 优先使用 C001-C003
- 不支持 audio driven → 使用 text+image（C004-C005）或 text（C006）
